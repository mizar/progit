...
この章は Git を始めることについてになります。最初にバージョン管理ツールにおけるいくつかの背景を説明することから始め、次にどのように Git をシステム上で稼動させるかに移り、最後は Git で作業を始めるためどのように設定するかです。この章のおわりには、なぜ Git が広まっているか理解し、なぜ Git を使うべきなのか、そして使うための準備が全て整っているはずです.
